...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
  ...
  def step(self, a):
    ...
    goal_dir = self.target_goal / np.linalg.norm(self.target_goal)
    xy_velocity = (xy_position_after - xy_position_before) / self.dt
    x_velocity, y_velocity = xy_velocity

    velocity = np.dot(np.array(x_velocity, y_velocity), goal_dir)
    ...
    if self.visualize_goal:
      idx = self.model.site_name2id("goal_site")
      pos = np.copy(self.target_goal)
      self.physics.data.site_xpos[idx][: len(pos)] = pos
...
class AntMazeEnv(maze_env.MazeEnv, GoalReachingAntEnv, offline_env.OfflineEnv):
  ...
  def __init__(self, goal_sampler=None, expose_all_qpos=True,
               reward_type='dense', reward_style='distance', v2_resets=False, list_resets=False, reset_list = [], mode=None,
               goal_distance=10,visualize_goal=False, reset_list_probs = None,
               *args, **kwargs):
    if goal_sampler is None:
      goal_sampler = lambda np_rand: maze_env.MazeEnv.goal_sampler(self, np_rand)
    self.goal_distance = goal_distance

    if len(reset_list) == 0 and mode is not None:
      if mode == 'y':
        reset_list = [[0,goal_distance]]
      elif mode == 'x':
        reset_list = [[goal_distance, 0]]
      elif mode == 'both':
        reset_list = [[goal_distance, 0], [0,goal_distance], [goal_distance, goal_distance]]

    assert (reward_style in ['distance', 'velocity'])
    
    self.reward_style = reward_style
    self.list_resets = list_resets
    self.reset_list = reset_list
    self.reset_list_probs = reset_list_probs
    if self.list_resets:
      assert (len(reset_list) > 0), "reset options must be specified"
      if self.reset_list_probs is not None:
        assert(len(self.reset_list_probs) == len(self.reset_list)), "reset list probs and reset list must match in weight"
    maze_env.MazeEnv.__init__(
        self, *args, manual_collision=False,
        goal_sampler=goal_sampler,
        expose_all_qpos=expose_all_qpos,
        reward_type=reward_type,visualize_goal=visualize_goal,
        **kwargs)
  ...
  def set_reset_list(self, reset_list):
    self.reset_list = reset_list
          
  def reset(self):
    if self.list_resets:
      self.set_list_target()
    elif self.v2_resets:
      """
    ...
    if self.visualize_goal:
      ...
      pos = np.copy(self.target_goal)
      self.physics.data.site_xpos[idx][: len(pos)] = pos
      #self.env.data.site_xpos[idx][: len(pos)] = pos
    return super().reset()
    
  def set_target(self, target_location=None):
    return self.set_target_goal(target_location)
  
  def set_list_target(self):
    # Weighted sample or uniform if reset_list_probs is None
    index = np.random.choice(len(self.reset_list), p=self.reset_list_probs)
    target_location = self.reset_list[index]
    return self.set_target_goal(target_location) 
