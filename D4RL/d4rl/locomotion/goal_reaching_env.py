...
class GoalReachingEnv(object):
  ...
  def _get_obs(self):
    ...
    if self.reward_style == 'velocity':
      obs = np.concatenate([base_obs, self._goal])
    else:
  ...
  def step(self, a):
    _, r, _, _ = self.BASE_ENV.step(self, a)
    if self.reward_style == 'velocity':
      reward = r
    else:
    ...
    if self.eval and self.reward_style == 'distance' and np.linalg.norm(self.get_xy() - self.target_goal) <= 0.5:
      done = True
