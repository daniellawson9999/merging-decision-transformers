...
def experiment(
    ...
    if variant['outdir']:
        variant['outdir'] = os.path.join(model_dir, model_type + '_' + exp_prefix)
    ...
    if variant['save_model']:
        if variant['share_input_output_proj']:
            model.predict_state = None
            model.predict_action = None
            model.predict_return = None
        if variant['save_mode'] == 'both' or variant['save_mode'] == 'last':
            torch.save(model,os.path.join(model_dir, model_type + '_' + exp_prefix + '.pt'))
            #torch.save(model.state_dict(),os.path.join(model_dir, model_type + '_' + exp_prefix + '.pts'))
        ...
        if variant['save_mode'] == 'both' or variant['save_mode'] == 'best':
            ...
            #torch.save(model.state_dict(),os.path.join(model_dir, model_type + '_' + exp_prefix + '_'  +'best' + '.pts'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpt_lm_const", type=float, default=1)
    parser.add_argument("--lm_const_linear_decay", default=False, action='store_true')
    parser.add_argument("--disable_k_annealing", default=False, action='store_true')
    ...
    parser.add_argument("--extend_positions", action="store_true", default=True)
