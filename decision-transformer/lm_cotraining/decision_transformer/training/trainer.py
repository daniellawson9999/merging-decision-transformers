...
class Trainer:
    def __init__(
        ...
        if self.init_reg is not None:
            self.init_state = copy.deepcopy(self.model.transformer.h.state_dict())
        else:
    ...
    def train_iteration(self, num_steps, iter_num=0, print_logs=False):
        ...
        if not self.eval_only:
            if self.args.get("outdir"):
                ...
                #     f"{self.args['outdir']}/model_{iter_num}.pts",
                # )
                if self.args['share_input_output_proj']:
                    predict_state = self.model.predict_state
                    predict_action= self.model.predict_action
                    predict_return = self.model.predict_return

                    self.model.predict_state = None
                    self.model.predict_action = None
                    self.model.predict_return = None
                ...
                # restore
                if self.args['share_input_output_proj']:
                    self.model.predict_state = predict_state
                    self.model.predict_action = predict_action
                    self.model.predict_return = predict_return
