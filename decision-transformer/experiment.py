...
from dt_utils import get_action, env_config, weight_average, load_model, weight_average_multi, setup_fisher_many, get_module_by_name
...
def experiment(
    ...
    if model_type == "dt":
        ...
        elif len(variant['pretrained_models']) == 1 and variant['pretrained_lm'] is not None:
            print("combining LM")
            source_model = torch.load(variant['pretrained_models'][0], map_location=device)
            naive = variant['naive']
            p = variant['p']
            if variant['flip']:
                print("flipping")
                source_model = weight_average(source_model, model, p, include=include)
                variant['pretrained_lm'] = None
                model = source_model
            else:
                model = weight_average(model, source_model, p, include=include)
            

        elif len(variant['pretrained_models']) > 0:

            if len(variant['pretrained_models']) == 1:
                old_model = load_model(variant['pretrained_models'][0],device=device)
            elif len(variant['pretrained_models']) == 2:
                old_model = load_model(variant['pretrained_models'][0], device=device)
                source_model = load_model(variant['pretrained_models'][1],device=device)
                naive = variant['naive']
                p = variant['p']

                if variant['use_fisher']:
                    print("Using FI exp")
                    assert (p==0.5), "changing p no effect, weights determined by info"
                    old_model= setup_fisher_many([old_model, source_model], variant, include)
                else:
                    old_model= weight_average(old_model, source_model, p, include=include)
                if variant['flip']:
                    assert (include is not None), "need params to flip"
                    ...
                    for name, param in source_model.named_parameters():
                        ...
                        for i in include:
                            if i in name:
                                ...
                                break
                        if copy_param:
                            param_source = get_module_by_name(source_model, name)
                            param_old = get_module_by_name(old_model, name)
                            param_source.data = param_old.data
                    # flip assignment
                    old_model = source_model
            elif len(variant['pretrained_models']) == 3:
                assert (not variant['flip']), 'not implemented'

                models = [load_model(variant['pretrained_models'][i], device=device) for i in range(len(variant['pretrained_models']))]
                if variant['use_fisher']:
                    print("Using FI exp")
                    old_model = setup_fisher_many(models, variant, include)
                else:
                    old_model= weight_average_multi(models, include=include)
            else:
                raise Exception("Not implemented for > 2")

            if variant['copy_model']:
                model = old_model
            elif variant['copy_include']:
                for name, param in old_model.named_parameters():
                        ...
                        for i in include:
                            if i in name:
                                ...
                                break
                        if copy_param:
                            param_source = get_module_by_name(old_model, name)
                            param_old = get_module_by_name(model, name)
                            param_source.data = param_old.data
            else:
                model.load_state_dict(old_model.state_dict())
        # Freeze 
        if include is not None and (variant['merge_frozen']):
            print("freezing!")
        ...
        if variant['disable_attention']:
            model.transformer.disable_attention = True
        else:
            model.transformer.disable_attention = False
    ...
    metric = f'target_{env_targets[0]}_return_mean'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--project', type=str, default="decision-transformer")
    ...
    parser.add_argument('--pretrained_models', default=[], nargs='+')
    parser.add_argument('--enable_merge_return', default=False, action='store_true')
    parser.add_argument('--enable_merge_embed_ln', default=False, action='store_true')
    parser.add_argument('--enable_timesteps', default=False, action='store_true')
    parser.add_argument('--enable_all_embed', default=False, action='store_true')
    ...
    parser.add_argument('--copy_model', default=False, action='store_true')
    parser.add_argument('--flip', default=False, action='store_true')
    parser.add_argument('--copy_include', default=False, action='store_true')
    parser.add_argument('--merge_frozen', default=False, action='store_true')
    parser.add_argument('--identity_layers', default=False, action='store_true')
